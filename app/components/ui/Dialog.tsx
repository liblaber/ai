  ...
  return (
    <button
      className={classNames('inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm transition-colors', {
        'bg-accent-500 text-white hover:bg-accent-600 dark:bg-accent-500 dark:hover:bg-accent-600': type === 'primary',
...
export const DialogTitle = ({ className, children, title, ...props }: RadixDialog.DialogTitleProps) => {
  return (
    <RadixDialog.Title
...
export const Dialog = ({ children, className, showCloseButton = true, onClose, onBackdrop }: DialogProps) => {
  return (
    <RadixDialog.Portal>
      ...
      <RadixDialog.Content asChild className="focus-visible:outline-none">
        <motion.div
          className={classNames(
          ...
          <div className="flex flex-col">
            ...
            {showCloseButton && (
              <RadixDialog.Close asChild onClick={onClose}>
                <IconButton className="absolute top-3 right-3 text-secondary hover:text-primary">
                  <X className="w-4 h-4" />
                </IconButton>
              </RadixDialog.Close>
            )}
